...
class TransferWorkingToPurchaseFirstTimeCron extends Command {
     ...
     * @var string
     */
    protected $signature = 'cron:transfer_working_to_purchase_first_time';
    ...
    public function handle() {
        ...
        $purchase_percentage = 100;//$projectSettings->purchase_percentage;
        $working_percentage = 0;//$projectSettings->working_percentage;

        $dashData = Dashboard::select('id','direct_income','roi_income','binary_income','supper_maching_income','working_wallet','working_wallet_withdraw','freedom_club_income')->where(DB::raw('ROUND(working_wallet-working_wallet_withdraw,2)'),'>',0)/*->limit(1)*/->get();
        $all_data=array();
        $all_dash_data=array();
        foreach ($dashData as $key => $req) {
            ...
            $second_time_balance = round(($req->working_wallet-$req->working_wallet_withdraw),2);
         
            echo "For User Id : ".$req->id."\n"; 

            $purchase_wallet_amount = round((($second_time_balance * $purchase_percentage)/100),2);

            $working_wallet_amount = round(($second_time_balance - $purchase_wallet_amount),2);

            $old_total_income_without_roi= 0;
            ...
            $insertData['balance'] = $second_time_balance;
            //$insertData->save();
            ...
            $updateData['working_wallet_withdraw'] = DB::raw('working_wallet_withdraw + '.$purchase_wallet_amount);
            $updateData['top_up_wallet'] = DB::raw('top_up_wallet + '.$purchase_wallet_amount);
            $updateData['second_time_balance'] = $second_time_balance;                    
        ...
        while($count3 <= count($array3))
        {
            ...
            $second_time_balance_qry = 'second_time_balance = (CASE id';;                    
            ...
            foreach ($arrProcess as $key => $val) {
                ...
                $second_time_balance_qry = $second_time_balance_qry . " WHEN ".$val['id']." THEN ".$val['second_time_balance'];
            ...
            $second_time_balance_qry = $second_time_balance_qry . " END)";
            ...
            $updt_qry = "UPDATE tbl_dashboard SET "/*.$working_wallet_qry." , "*/.$working_wallet_withdraw_qry." , ".$top_up_wallet_qry." , ".$second_time_balance_qry." , ".$working_to_topup_qry." , ".$total_income_without_roi_qry." , ".$old_total_income_without_roi_qry." WHERE id IN (".$ids.")";
