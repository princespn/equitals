              ...
              <div class="container-fluid">
                <div class="card card-payment">
                  ...
                  <div class="card-header">
                    <h4 class="card-title">All ID Balance Receive</h4>
                  </div>
...
export default {
  ...
  methods: {
    getTopupReport() {
      ...
      setTimeout(function () {
        const table = $("#balance-report").DataTable({
          responsive: true,
          lengthMenu: [
            [10, 50, 100],
            [10, 50, 100],
          ...
          bFilter: false,
          ajax: {
            url: apiUserHost + "all-id-balance-report",
            type: "POST",
            data: function (d) {
              i = 0;
              i = d.start + 1;
    ...
    transfer_balance() {
      ...
      axios
        .post("add-transfer-request", { amount: this.withdraw_balance })
        .then((resp) => {
          if (resp.data.code == 200) {
            this.$toaster.success(resp.data.message);
            $("#editBankDetailsmodal").modal("hide");
            this.$router.push({ path: "pending-balance-transfer-requests" });
          } else {
    ...
    checkRequestExists() {
      axios
        .post("check-transfer-request-exists")
        .then((resp) => {
          if (resp.data.code == 200) {
            this.$toaster.error(resp.data.message);
            this.$router.push({ path: "pending-balance-transfer-requests" });
