    ...
    <div class="content">
        <div class="">
            <div class="page-header-title">
                <h4 class="page-title">Confirm Hotel Bookings</h4>
            </div>
        </div>
            ...
            <div class="container">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="panel panel-primary">
                            <div class="panel-body">
                                <table id="confirmhotel-orders" class="table table-striped table-bordered dt-responsive" style="width: 100% !important;">
                                    <thead>
                                        <tr>
                                            <th>Sr.No</th>
                                            <th>Order Id</th>
                            ...
                            <div class="col-md-12">
                                <div class="col-md-12">
                                    <h4><span class="text-primary">Hotel Location </span>: <span class="text-muted">{{hotel_loc}}</span> 
                                         ...
                                         <h4><span class="text-primary">Postal Code </span>: <span class="text-muted">{{postalCode}}</span> 
                                        </h4>
    ...
    export default {
        ...
        methods: {
            ...
            codOrders(){
                ...
                setTimeout(function(){
                    const table = $('#confirmhotel-orders').DataTable({
                        responsive: true,
                            ...
                            // },
                            {
                                render: function (data, type, row, meta) {
                                    if(row.status == 'confirm')
                                        ...
                                        return '<label class="text-default">'+row.status+'</label>';
                                    else
                    ...
                    $('#confirmhotel-orders').on('click', '#view-detail',function(){
                        that.viewOrderDetails($(this).data('id'));
                    });
                    /*  Appprove details */
                    $('#confirmhotel-orders tbody').unbind('click').on('click', '#onApproveClick', function (event){
                        var data = table.row($(this).parents('tr')).data();
                        var id = $(this).data('id');
                        var remark = '';//$('#remark_' + id).val();
                        that.onApproveClick(id,remark);
                    });
                    $('#confirmhotel-orders tbody').on('click', '#onRejectClick', function () {
                        var data = table.row($(this).parents('tr')).data();
                        var id = $(this).data('id');
                ...
                if(remark == ''){
                    ...
                    if (result.value) {
                        axios.post('approveBookingRequest', {
                            ...
                            $('#confirmhotel-orders').DataTable().ajax.reload();
            ...
            exportToExcel(){
                var params = {status : 'confirm',action:'export',responseType: 'blob' };
                axios.post('get-hotel-booking', params).then(resp => {
                    if (resp.data.code == 200) {
                        ...
                        fileLink.setAttribute('download', 'Confirm-Hotel-Booking.xls');
