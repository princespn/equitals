	    ...
	    <div class="container">
	        ...
	        <div class="row">
	            <div class="col-md-12">
	                <div class="panel panel-primary">
	                    <div class="panel-body">
	                        <table id="touprequest-report11" class="table table-striped table-bordered dt-responsive">
	                            <thead>
	                                <tr>
	                                    <th>Sr.No</th>
	                                    <th>User Id</th>
...
<div class="modal" tabindex="-1" role="dialog" id="dispalyimagemodal">
    ...
    export default {
        ...
        methods: {
            ...
            topupRequestReport(){
                ...
                setTimeout(function(){
                    const table = $('#touprequest-report11').DataTable({
                        responsive: true,
                        retrieve: true,
                        destroy: true,
                        processing: false,
                    ...
                    $('#onResetClick').click(function () {
                      ...
                      /*  Appprove details */
                    $('#touprequest-report11 tbody').unbind('click').on('click', '#onApproveClick', function (event){
                    ...
                    $('#touprequest-report11 tbody').on('click', '#onRejectClick', function () {  
                       //    var data = that.table.row($(this).parents('tr')).data();
                       // var remark = $('#remark_' + data.id).val();   
                        that.onRejectClick($(this).data('id'));
                    });

                    $('#touprequest-report11').on('click', '#onImageClick', function () {
                    ...
                    if (result.value) {
                        axios.post('approve-topup-request', {
                            ...
                            payment_type:payment_type,
                              device:'web',
                           topupfrom:'confirm manual TP Admin'
                        }).then(response => {
                            if(response.data.code === 200) {
                                this.$toaster.success(response.data.message);
                                $('#touprequest-report11').DataTable().ajax.reload();
